...
**There is no pose in this kit where anybody is holding a gun up.** This used to
say there was no combat at all, and that was wrong: the character art is
[LPC Revised](https://github.com/ElizaWy/LPC), pinned at one commit, and that
commit has always shipped *Combat 1h* — Idle, Slash, Halfslash, Backslash —
for every body, head, garment and hairstyle. They were out of reach rather than
missing, because the people were a pinned sheet nothing could rebuild; they are
built again now (see *Everybody is made of components*, below), and the combat
set is in them. But it is a set for a SWORD: squared up, a forehand, a backhand,
and the one frame with the arm held straight out is the middle of a lunging
punch — the head travels fourteen pixels. The pose most LPC games with guns use
is the universal spritesheet's bow `shoot` row with a rifle drawn over it, and
that is on a different and larger body, offered as CC-BY-SA 3.0 or GPL 3.0 — a
licence this project will not mix into OGA-BY art, for the reason set out in
LICENSE part 2. So there was never going to be a shoot row to drop in.
...
**A swing is not an arm any more — it is the kit's own.** The two things you
swing are held in LPC Revised's one-handed combat set, which the people are
built with now: squared up while it is out, feet apart, and the kit's forehand
and backhand while it goes round, three frames each — swingDir alternates them
exactly as it always alternated the arc. That is how every LPC game with a sword
does it: the body plays the swing and the weapon is a layer in the hand. The
frame draws the arm, so nothing is cut; `Guns.HANDS` is where the hands are in
each of those frames, measured off the composed art (the only body pixels a
dressed person shows below the head are the hands), and the weapon goes in
whichever one is leading along its bearing at that instant. A gun is still held
on a turned arm, because a gun is aimed at any angle and a sword is swung at
one.
...
went. And a face belongs to a **person**: the faces are built in the same run as
the people, handed the pixels just baked, and which of the three heads and which
skin each colleague has comes off the base their `look:` names — so Bev's mouth
is Bev's mouth, and there is no second roster to disagree with the first. The
build still checks each face against the pixels it was handed and stops if
somebody else's fits better.
...
## Five voices, and they were all one stream

The office talks to you constantly. A yoghurt thread in `#general`, a password
that expired before the reminder about it arrived, a phone abandoned on a floor
you are not on, an achievement, twelve minutes lost to a Greggs. Five different
kinds of thing from five different senders, and for a year every one of them
arrived the same way: one line of grey text into one box in the corner.

Two of the five had somewhere to go back to — the chat and the inbox were tabs
eight and nine of a ten-tab self-service portal. The other three had nowhere at
all, because **the box was the archive**, and that is the fault underneath every
other fault. A notification is a tap on the shoulder and an archive is a place,
and one element cannot be both. So it had to be a fixed window onto a
thirty-entry scrollback, 196 pixels tall on a desktop and 34 on a phone. It had
to fade itself out, because a box that size sitting over the office all shift
reads as a wall dropped over the game. It had to grow a little chip to stand in
for itself while it was faded, because a box with no hit area is a box you
cannot get back to. And after all of that the answer to *what was that email*
was still to scroll the whole afternoon backwards through four other kinds of
thing.

There are two things now, and the split is the whole point.

**The record** is five channels and each one is genuinely the thing it says it
is. **Mail** is an inbox: threads by who wrote to you, headers, and the body in
the paragraphs it was written in — which matters, because about half the comedy
in this game's inbox is in the quoted reply chain and the signature. **Texts**
are bubbles, and yours are on the right, because it is the only conversation in
the building with two sides to it. **Chat** is channels. **The log** is a
time-ordered ledger with the hours marked down the side, and it is the only one
of the five with no list beside it, because a ledger has nothing to choose
between — it has an order. **Calls** is one transcript per call, which is new:
the turn-by-turn record a call kept while it was happening used to be thrown
away with the card the moment you pressed *End call*.

**The alert** is a pointer into that and nothing else. One line, coloured and
iconed by which channel it came from, at most two of them on a phone, gone in
five seconds — and tapping one opens that channel at the thing it just told you
about. It carries no history, which is exactly what lets it be two lines high
instead of a quarter of the screen. Nothing fades, nothing stands in for
anything, and the way back to the record is a control that is always there: five
chips in the corner on a desktop, one chip carrying the total on a phone, where
five 34-pixel targets do not fit the 160 pixels the band has on a 320-pixel
screen.

None of it grows with traffic, and that is arithmetic rather than a hope. The
band's tallest possible state is two rows and a gap, declared once in the
stylesheet and read back by the code that caps the stack, so the two can never
disagree about how many fit. Twelve things happening in one second is twelve
lines in the log and two alerts on the screen.

### How often, which is the harder half

A small pop-up every three seconds is still a pop-up every three seconds, and
the first version of this fixed the footprint and left the frequency alone.
Measured over one shift it raised **166 alerts and 111 of them were the office
chat** — a yoghurt thread, one pop-up per line.

The clock is what makes it hard. A game minute is 430 milliseconds, so the whole
day's correspondence — every email, text and incident, all correctly timed
across eight hours — arrives inside about three and a half real minutes. At that
compression any per-item pop-up is a stream however well sorted.

So three rules. **Not every channel is worth interrupting for**: mail and texts
are somebody addressing you and waiting; the log is the game answering what you
just did; the chat and the call log are neither, so they go to the rail quietly
and light a count. **A burst is one row with a number on it** — six emails at
once is "✉️ 6 new", the row does not move while it happens, and the chime does
not fire again, because a chime every four seconds *is* the spam. And **there is
a floor of nine real seconds between pop-ups**, with anything arriving inside it
queued rather than dropped and released one channel at a time. The single
exception is a message waiting on an answer from you, which is the only arrival
in this game with a cost attached to ignoring it.

**Seven pop-ups in a real minute of play against nineteen, one row on screen at
a time, about two dozen across a shift against a hundred and sixty-six — and
nothing lost.** Every line is in the record.

And it is a setting, because past a certain point it is a preference: **☰ · Menu
→ Notifications** is *everything*, *only what needs you*, or *nothing*. The last
of those is only safe to offer because the counts on the rail still tell you
what arrived.

## Email is not a phone call, and the game now knows

The turn-based argument at the centre of this game was always a correspondence
with a stopwatch on it. You pick a reply, they reply, their frustration moves,
your patience moves. For two years it was dressed exclusively as a phone call,
because a phone call was the only thing in the game.

It is three channels now, and no mechanic changed. What changed is what the card
says it is — *"Incoming call · queue position 4"* over an email is a lie you can
read, and *"♪ hold music"* under a text message is worse than no line at all.
An email exchange says how long it has been sitting in your inbox and shows you
the auto-reply that went out on the thread. A text says *typing…*, and then
*read 09:41 · no reply yet*. The meter that is how long a caller will hold is,
in writing, how many more replies they will read before this becomes somebody
else's problem: the same number, honestly labelled.

And the work arrives differently, which is the part only this version of the
game could have. **A phone rings on the fourth floor and stops ringing when you
leave the building.** An email does not. So there is a second queue: a formal
complaint on its second attempt, a reply-all thread with a deck attached, a form
somebody typed into a box at ten to three in the morning, a text from a man a
colleague called Ryan gave your number to. They arrive wherever you are, there
is no forty-two-second abandon timer on any of them, and one can land while you
are standing on a beach on the far side of the island. Because it has been
composed rather than said, it comes in angrier than any phone call and lands
softer per turn: prose cannot shout at you in real time, it can only be read
again.

What it costs to ignore one is a line on the shift report rather than a phone
going quiet in front of you — which is the difference between the two queues,
and the reason the HUD says both. The ringing one is amber and it blinks. The
written one is blue and it does not.
...
| `engine/comms.js`, `css/comms.css` | The five channels the office talks to you in, the one-line alert that points into them, and the console you read them in. Nothing in it grows with traffic — see **Five voices**. |
...
| `scripts/check.sh` | Checks the build and moves the version string: every script parsed, every level built and walked, the streamer, the parts, the maps, and every file the pages reference. Run it before you ship. |
| `scripts/release.sh` | Publishes the released files into the public repository as a single commit. Run by `.github/workflows/release.yml` on every push to `main`, after the checks. |
...
`sanitary`, `world`, `cars` — went through a curation pass (hand-picked seamless
crops) that predates this tool, or arrived whole, and were never captured as
code. It cannot rebuild them, so it doesn't try: `tools/lib/pinned.mjs` reads their entry
...
Three kinds of managed sheet, and a sheet says which it is with `kind:`. A sheet
of THINGS is crops — the town — and is what a sheet declares by saying nothing.
The PEOPLE are layers composited into people (`kind: 'people'`,
`tools/lib/buildPeople.mjs`), and the expressions sheet is measured against
those people in the same run rather than cropped (`kind: 'faces'`). A fourth
kind would be a line in `tools/build-sprites.mjs` and nothing else.

### Everybody is made of components

`tools/sheets/people.mjs` is the whole wardrobe: nine skins on two builds with
two heads, six eye colours, fourteen haircuts and two beards in eleven colours,
four shirts in eighteen, a cardigan, two cuts of trouser and a pair of shoes —
four hundred and fourteen components, each one directory of LPC Revised at the
pinned commit. A person is a `look:` in `data/npcs.js` naming components in
that catalogue, which is also exactly what the character creator writes, and
the player's starting look is `PLAYER_LOOK` beside them.

- **The cast is baked.** The build reads `data/npcs.js`, composites every look
  and writes `art/sprites/people.png`, one row each. It reproduces the pinned
  sheet it replaced to the pixel — the recipe for that sheet had been lost in a
  rewrite, and was recovered from history and checked against it.
- **The creator's components are one small PNG each** in `art/sprites/parts/`,
  fetched only when somebody wears one. A player who takes the default fetches
  none: the build baked that person already.
- **The data is what is drawn.** Each baked row records the look it was made
  from; a colleague whose look has changed in the data since is dressed at
  runtime from the same components.
- **The cells are 64 wide**, the whole of upstream's frame, because the combat
  frames swing the arm out to its edge. The centre did not move, so a person
  stands exactly where they always did.
