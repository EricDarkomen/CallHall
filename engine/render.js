  ...
  floorTile(z, v) {
    ...
    if (Tiles.has(kit)) {
      return this._bake('k' + z + v + kit, (g, N) => {
        const r = Tiles.rects[kit];
        g.imageSmoothingEnabled = false;
        g.drawImage(Tiles.img, r[0], r[1], r[2], r[3], 0, 0, N, N);
  ...
  draw(dt) {
    ...
    for (let y = y0; y <= y1; y++) for (let x = x0; x <= x1; x++) {
      ...
      const nz = (World.zone[y + 1] && World.zone[y + 1][x]) || (World.zone[y - 1] && World.zone[y - 1][x])
        || World.zone[y][x + 1] || World.zone[y][x - 1] || null;
    ...
    drawables.forEach(d => {
      if (d.kind === 'counter') {
        ...
        if (onFloor && !(f.sprite && Tiles.has(f.sprite)) && o.kind !== 'chair' && o.kind !== 'bin' && o.kind !== 'hatch') {
        ...
        if (!(f.sprite && Tiles.draw(c, f.sprite, ex, ey + bob))) {
        ...
        if (this.questMark(n)) this.emoji('❗', at.x + 13, box.y - 4, 15);
        if (sprite) {
