...
const Tools = {
  ...
  init() {
    ...
    document.addEventListener('touchstart', e => {
      letScroll = !!(e.target && e.target.closest
        && e.target.closest('.pane, .barrow, .tabs, textarea, input, select, #edAsk'));
    }, { passive: true });
  ...
  doEntry(t, e) {
    if (e.shiftKey) {
      Ask.form('New arrival point', [
        { k: 'name', label: 'called', value: 'start',
    ...
    const name = Sel.kind === 'entry' ? Sel.name : Object.keys(Doc.entries)[0];
    if (!name) {
      Side.say('No arrival point to move. Shift-click to add one.');
      return;
    }
