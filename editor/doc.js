...
const Doc = {
  ...
  base: null,
  /* Every edit pushes the state before it. Bounded because an unbounded undo
     stack on a few hundred objects is a slow leak nobody notices. */
  ...
  /* Call BEFORE mutating. Every tool does; `label` is what the undo button
     says it would take back. */
  mark(label) {
  ...
  changed() { return JSON.stringify(this.state()) !== JSON.stringify(this.base); },
...
/* Structured clone by hand: the doc is JSON-shaped by construction (the DERIVED
   list is what keeps functions and cyclic build output out of it), and this
   works on every browser without asking whether structuredClone exists. */
function clone(v) {
