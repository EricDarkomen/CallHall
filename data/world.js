...
const ZONES = {
  ...
  /* And two more, because the estate grew two streets the day it grew a road
     network: it was seven parallel avenues joined to nothing, and it is a loop
     road with six streets off it now — three legs and six avenues is nine
     names for seven. See data/roads.js. The two closes are the two the layout
     makes dead ends of, and Sycamore Close was already called a close while
     being a through road, which is the sort of thing nobody notices until the
     roads are laid out by something that knows what a close is. */
  chestnut:  { name: 'Chestnut Grove',      floor: '#4a4e56', alt: '#45494f', wall: '#33373d', tint: '#9fb3c8', surf: 'concrete', wsurf: 'block', tile: 'terrain.slab', wtile: 'wall.brick' },
  laburnum:  { name: 'Laburnum Close',      floor: '#4a4e56', alt: '#45494f', wall: '#33373d', tint: '#9fb3c8', surf: 'concrete', wsurf: 'block', tile: 'terrain.slab', wtile: 'wall.brick' },
...
const SURFACES = {
  ...
  rock: {
    /* The cliff, and what there is of the foreshore under it. This used to
       be the kit's stone wall laid flat and tinted grey off the brick red
       it ships in — except floorTile()'s multiply tint only ever lightens
       a colour, so the brick could be dimmed but never actually turned
       grey, and a wall laid on its side is still a wall. `surf: 'rock'` is
       drawn instead — see the 'rock' case in floorTile() — which is real
       grey because nothing about it started out red. */
    surf: 'rock',
