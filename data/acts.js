...
const Acts = {
  ...
  /* --- the working day --- */
  playerDesk() {
    if (G.minutes >= DAY_END) return insp('🖥️', 'Your workstation', 'Shift over', ['The shift is over. The screen has already logged you out. It did that at 17:00:00.']);
  ...
  cables() { insp('🔌', 'Cable spaghetti', 'Do not pull', ['A knot the size of a dog. Somewhere in it, one cable is not connected to anything and has not been for a long time.']); },
  ...
  oldCall() {
    if (G.flags.answeredOld) return insp('📟', 'Terminal', 'Line clear', ['The line is clear. For the first time since 2009, the display says: ACTIVE CALLS: 0.']);
  ...
  beanbag() {
    ...
    insp('🛋️', 'The beanbag', 'Procured 2021 · used 3 times', [
      ...
      'You sit in it. It makes a noise like a slow avalanche. Getting out will be its own event.',
  ...
  birthdayCard() {
    ...
    insp('🎂', 'The birthday card, circulating', 'For Marcus', [
      ...
      [{ t: 'Sign it: “Happy birthday!”', to: null, do() { G.flags.signedCard = true; Player.mod({ rep: 1 }); UI.toast('🎂', 'You add a name to a card for a man you have never met. This is how community works and it is fine.'); } },
       { t: 'Sign it: “Marcus — I hope you are real.”', to: null, do() { G.flags.signedCard = true; G.flags.cardWeird = true; P.stats.chaos += 2; Player.xp(15); UI.toast('🎂', 'You write it before you can stop yourself. It is going to be read out. It is always read out.'); } },
  ...
  trolley() { insp('🧽', "Bev's trolley", 'Parked, not abandoned', [
